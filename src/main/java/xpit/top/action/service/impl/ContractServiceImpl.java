...
import xpit.top.action.domain.vo.HighestBidderInfoVo;
...
public class ContractServiceImpl implements ContractService {
    ...
    public ResponseResult contractDeploy() throws CipherException, IOException {
        ...
        return ResponseResult.okResult(contractAddress);
    }

    @Override
    public ResponseResult contractLoadAddProduct(Product product) throws Exception {
        TransactionReceipt transactionReceipt = ecommerceStore().addProductToStore(product.getName(),product.getCategory(), product.getImageLink(), product.getDescLink(), DateUtils.dateToTimestampe(product.getAuctionStartTime()), DateUtils.dateToTimestampe(product.getAuctionEndTime()), product.getStartPrice(), product.getCondition()).send();
